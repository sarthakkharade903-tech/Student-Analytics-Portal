...
export default function PerformanceChart({ data }: { data: ChartData[] }) {
  if (!data || data.length === 0) {
    return <div className="h-full flex items-center justify-center text-white/40 text-sm">No data available</div>
  }
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-[#0a0f1c]/95 backdrop-blur border border-white/10 rounded-xl p-3 shadow-xl">
          <p className="text-sm font-semibold text-white/90 mb-1">{label}</p>
          <p className="text-[10px] text-white/50 mb-2 uppercase tracking-wider">
