...
export default function SubjectPerformance({ scores }: { scores: any[] }) {
  ...
  if (!analytics) {
    return (
      <div className="glass-card rounded-3xl p-6 lg:p-8 flex flex-col items-center justify-center min-h-[300px] border border-gray-200 bg-white/80 text-center shadow-sm">
        <Target className="w-12 h-12 text-[var(--muted-foreground)] opacity-50 mb-4" />
        <h3 className="text-lg font-bold text-gray-900 mb-2">Subject Performance</h3>
  ...
  return (
    <div className="glass-card rounded-3xl p-6 lg:p-8 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden bg-gradient-to-br from-white to-gray-50 border border-gray-200 shadow-sm">
      ...
      <div className="w-full md:w-1/2 flex flex-col items-center justify-center relative z-10">
        <h3 className="text-lg font-bold mb-2">Subject Performance</h3>
        ...
        <div className="w-full h-64 relative drop-shadow-2xl">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.9)', borderRadius: '12px', border: '1px solid rgba(0,0,0,0.1)' }}
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none" style={{ marginTop: '-36px' }}>
            <div className="text-center">
              <span className="text-2xl font-bold">{Math.round(pieData.reduce((a, b) => a + b.value, 0) / pieData.length)}%</span>
      ...
      <div className="w-full md:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-4 relative z-10">
        ...
        {/* Strongest Subject */}
        <div className="bg-white border border-gray-200 rounded-2xl p-4 hover:bg-gray-50 hover:border-green-500/30 transition-all group shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-green-500/20 flex items-center justify-center">
              <Award className="w-4 h-4 text-green-600 group-hover:scale-110 transition-transform" />
            </div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">Strongest</h4>
          </div>
          <p className="text-lg font-bold text-gray-900">{strongest.name}</p>
          <p className="text-sm text-green-600 font-medium">{Math.round(strongest.avgPct)}% Avg</p>
        </div>

        {/* Needs Improvement */}
        <div className="bg-white border border-gray-200 rounded-2xl p-4 hover:bg-gray-50 hover:border-yellow-500/30 transition-all group shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-yellow-500/20 flex items-center justify-center">
              <AlertTriangle className="w-4 h-4 text-yellow-600 group-hover:scale-110 transition-transform" />
            </div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">Needs Work</h4>
          </div>
          <p className="text-lg font-bold text-gray-900">{weakest.name}</p>
          <p className="text-sm text-yellow-600 font-medium">{Math.round(weakest.avgPct)}% Avg</p>
        </div>

        {/* Most Improved */}
        <div className="bg-white border border-gray-200 rounded-2xl p-4 hover:bg-gray-50 hover:border-blue-500/30 transition-all group shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-blue-500/20 flex items-center justify-center">
              <TrendingUp className="w-4 h-4 text-blue-600 group-hover:scale-110 transition-transform" />
            </div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-[var(--muted-foreground)]">Improved</h4>
          </div>
          <p className="text-lg font-bold text-gray-900">{mostImproved.name}</p>
        ...
        {/* Focus Area */}
        <div className="bg-gradient-to-br from-violet-50 to-fuchsia-50 border border-violet-200 rounded-2xl p-4 hover:from-violet-100 transition-all group shadow-[0_0_15px_rgba(139,92,246,0.05)]">
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-lg bg-violet-500/20 flex items-center justify-center">
              <Target className="w-4 h-4 text-violet-600 group-hover:scale-110 transition-transform" />
            </div>
            <h4 className="text-xs font-semibold uppercase tracking-wider text-violet-700">Focus Area</h4>
          </div>
          <p className="text-lg font-bold text-gray-900">{weakest.name} Concepts</p>
          <p className="text-xs text-gray-600 mt-1 line-clamp-1">Try focusing on basic {weakest.name.toLowerCase()} topics.</p>
        </div>
