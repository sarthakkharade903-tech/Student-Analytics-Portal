              ...
              >
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="font-medium text-gray-900">{score.test_name}</p>
                      <p className="text-[10px] text-gray-500 mt-0.5 uppercase tracking-wider">{score.test_type}</p>
                    </div>
                  </div>
                </td>
                ...
                <tr className="bg-gray-50">
                  <td colSpan={6} className="p-0 border-t border-gray-200">
                    <div className="p-6 animate-in fade-in slide-in-from-top-2">
                      <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-4">Subject Breakdown</h4>
                      
                      {score.is_absent ? (
                        <div className="text-sm text-gray-500 italic flex items-center justify-center py-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                          {score.subjects.map((sub) => (
                            <div key={sub.name} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
                              <div className="flex justify-between items-end mb-3">
                                ...
                                <div className="text-right flex flex-col items-end gap-1">
                                  <div className="flex items-baseline gap-1.5">
                                    ...
                                    {sub.max_marks > 0 && (
                                      <span className="text-xs text-gray-400 font-normal">/ {sub.max_marks}</span>
                                    )}
                                  </div>
                                  <div className="flex items-center gap-2">
                                    <span className="text-[10px] text-gray-500">({sub.percentage}%)</span>
